...
| `volatile float* cache` warp-synchronous LDS | *no equivalent* | See §2.3 — the barrier-free tail is done in registers via shuffles instead. |
...
| plain pointer loads | *buffer* tensors (`rocdl.make_buffer_tensor`) | Carries a resource descriptor, so accesses are hardware bounds-checked. This is why the GEMM prefetch can read one tile past the end without faulting — **within a row**; see §2.5 for where that guarantee stops. |
...
| 80 SMs (V100) | **256 CUs** | Grid sizes tuned for a V100 leave a MI350X three-quarters idle. The reduce ladder's fixed 1024-block grid and the GEMM's 128×128 tile at M=N=1024 both hit this. |
...
- FP32 **matrix** (MFMA) ≈ 144 TFLOP/s — 2× the vector pipe
...
launch. Every reduce variant measured **32.0 µs ± 0.3** regardless of grid size,
...
`v_pk_fma_f32`, no scratch, and **10.6 TFLOP/s** — a 6× speedup from one call.
...
| A (16×4) | `A[l % 16][l / 16]` |
| B (4×16) | `B[l / 16][l % 16]` |
| D/C (16×16), 4 VGPRs | `D[4*(l/16) + r][l % 16]`, `r ∈ 0..3` |

This was validated with a 16×16×4 single-wavefront kernel against `A @ B` before
...
M=N=K=1024 the 128×128 GEMM tile produces a 8×8 = **64-block** grid on a 256-CU
part, and the 16×16 tile (4096 blocks) beats it — 14.3 vs 10.5 TFLOP/s. The
ordering inverts at 4096³ (13.9 vs 68.8). The V100-tuned tile shape is not wrong;
...
uniform matrix that verdict holds exactly (0.72× — it is pure overhead). On a
power-law matrix the same kernel is **4.6× faster** than v0. The win is not the
...
land within 1% of each other at ~69.5 TFLOP/s at 4096³ — about 97% of the FP32
...
hardware asks for, and `v5_mfma` takes it: **114 TFLOP/s**, 1.65× the best
