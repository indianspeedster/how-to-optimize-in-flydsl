...
"""Rung 0 -- one f32 per lane. Ports the ``add`` kernel of
``elementwise/elementwise_add.cu``.

The baseline: each lane loads one float from A, one from B, stores one to C.
Lowers to ``buffer_load_dword``. C = A + B reads 2N and writes N floats and does
one add per 12 bytes, so this is pure HBM traffic and the only metric that means
anything is achieved bandwidth.
"""
