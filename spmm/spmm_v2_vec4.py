...
"""Rung 2 -- four output columns per thread. **No CUDA counterpart.**

The sparse row walk is untouched; only the dense side gets wider. Each thread
now owns four columns of B and reads them with ``buffer_load_dwordx4``.

Since the dense side is essentially all of the traffic, this is where the win is:
1.7x at n=256 and 2.4x at n=1024, reaching 6202 GFLOP/s.

It does *not* help on the power-law matrix (1.13x) -- there the bottleneck is
load imbalance between rows, which a wider transaction cannot touch. v1 is the
rung that addresses that one.
"""
...
VEC = 4            # output columns each thread owns
