...
|---|---|---|---|---|
| `v0_float` | one f32 per lane | 5283 GB/s | 5127 GB/s | 0.88× |
| `v1_float2` | two f32 per lane | 5649 GB/s | 5381 GB/s | 0.92× |
| `v2_float4` | four f32 per lane | **5923 GB/s** | **5869 GB/s** | **1.00×** |
| `v3_float4_x4` | 4× float4 per lane | 5574 GB/s | 5597 GB/s | 0.94× |
...
| rung | | N=32M | speedup |
|---|---|---|---|
| `v0_baseline` | LDS tree, `tid % (2s) == 0` — divergent | 1704 GB/s | 1.00× |
| `v1_no_divergence` | `index = 2*s*tid`, active lanes packed | 1739 GB/s | 1.02× |
| `v2_no_bank_conflict` | `s` halving, `tid < s` | 1928 GB/s | 1.13× |
| `v3_add_during_load` | fold 2 globals before the tree | 3637 GB/s | 2.13× |
| `v4_unroll_last_wave` | last 64 lanes finish in registers | 4001 GB/s | 2.35× |
| `v5_full_unroll` | LDS levels emitted straight-line | 4021 GB/s | 2.36× |
| `v6_multi_add` | serial accumulate, then the tree | 6861 GB/s | 4.03× |
| `v7_shuffle` | serial accumulate + wave shuffle | **6940 GB/s** | **4.07×** |
| `v8_shuffle_vec4` | v7 with dwordx4 loads | 6862 GB/s | 4.03× |
| `v9_vec4_wide_grid` | v8 on 32 blocks/CU | 6825 GB/s | 4.01× |

**6.94 TB/s is 87% of the 8 TB/s HBM peak, and 1.33× PyTorch's segmented sum.**
...
|---|---|---|---|
| M=16384, N=16 | `v2_subwave_per_row` (4 rows per wavefront) | 154 | **1.81×** |
| M=16384, N=64 | `v0_wave_per_row` | 588 | **1.80×** |
| M=16384, N=256 | `v0_wave_per_row` | 2298 | **1.89×** |
| M=16384, N=4096 | `v3_block_per_row` | 7419 | **1.56×** |
| M=1024, N=16384 | `v3_block_per_row` | 6237 | **1.95×** |
...
| `v1_lds_tile` | 16×16×16 LDS tile | 14.2 | 15.1 | 13.8 |
| `v2_thread_tile` | 128×128×8, 8×8 per thread, float4 | 10.5 | 42.1 | 68.7 |
...
TFLOP/s. At 4096³ the ladder spans **14×** end to end and finishes at **84% of
rocBLAS**.

Three things worth reading off this table:

- **v2–v4 land within 1% of each other at 4096³** — about **97% of the FP32
...
- **The only move left is the one the hardware asks for.** The CUDA repo's final
  ...
  FLOP/clock/CU against the vector pipe's 128. `v5` takes it and gets 1.65× the
  best vector rung.
- **At 1024³ the register-tiled kernel loses to the naive LDS tile.** A 128×128
  tile gives an 8×8 = 64-block grid on a 256-CU part. The V100-tuned tile shape
...
on a power-law matrix a **full wavefront per row is 13.8× the scalar kernel**,
because the wavefront is what absorbs the load imbalance. All three shapes beat
PyTorch's CSR `mv` (up to 48×; it is not a tuned path).
...
**On a uniform matrix that verdict holds exactly** (0.72×, pure overhead). **On a
power-law matrix the same kernel is 4.6× faster than v0** — and the win is not the
...
in particular §2.2 (host dispatch masquerading as a bandwidth roof), §2.3
(shuffles inside `scf.if` are silently wrong), and §2.7 (`a*b + c` is not an FMA
and costs 6×).
