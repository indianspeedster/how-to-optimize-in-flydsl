...
"""Rung 8 -- v7 with dwordx4 loads. **No CUDA counterpart, and no gain.**

The hypothesis: CDNA4 pulls 128 bytes per line into L1, so a 128-bit
``buffer_load_dwordx4`` per lane should saturate it better than a dword.

It does not. 6862 GB/s against v7's 6940 -- inside the noise, if anything
slightly worse. Once each thread accumulates serially the kernel is bound by HBM,
not by how many transactions it takes to get there.
