...
"""Rung 0 -- one lane per row. Ports ``spmv/spmv.cu`` with
``THREADS_PER_VECTOR = 1``.

The classic scalar CSR kernel: thread ``i`` walks row ``i`` end to end. No
cross-lane reduction at all, which sounds efficient and is not: within a
wavefront the 64 lanes walk 64 *different* rows, so their gathers into x are
uncorrelated and a single long row stalls the whole wavefront.

The baseline the knob is swept against.
"""
