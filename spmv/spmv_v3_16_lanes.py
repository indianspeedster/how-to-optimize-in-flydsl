...
"""Rung 3 -- 16 lanes per row. Ports ``spmv/spmv.cu`` with
``THREADS_PER_VECTOR = 16``.

Sixteen lanes per row. Best setting at 32 nnz/row (309 GFLOP/s): roughly two
non-zeros per lane, which is where the gather coalescing has paid off and the
reduction has not yet started to dominate.
"""
