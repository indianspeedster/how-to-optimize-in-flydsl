...
"""Rung 1 -- 4 lanes per row. Ports ``spmv/spmv.cu`` with
``THREADS_PER_VECTOR = 4``.

Four lanes share a row. The gathers of the four lanes now walk the *same* row,
so they are adjacent in ``col_index`` and the loads coalesce; the cost is a
2-step shuffle reduction per row. Worth 2.8x at 32 nnz/row.
"""
