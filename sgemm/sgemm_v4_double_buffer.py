...
"""Rung 4 -- halve the barriers. Ports the double-buffering half of
``sgemm/sgemm_v3.cu`` (``ENABLE_DOUBLE_BUFFER``).

Two LDS buffers instead of one: this tile's math reads buffer p while the next
tile is written into buffer 1-p, so the second barrier per K-tile disappears.

Also worth nothing at 4096^3 (68.8 TFLOP/s). Together with v3 that is the
finding: **the vector-FMA ladder is finished**. v2, v3 and v4 land within 1% of
each other, at ~97% of the FP32 vector peak. Latency hiding cannot help a kernel
that is not stalling. The only move left is to stop using the vector pipe --
which is v5.
"""
...
def build(M: int, N: int, K: int):
    if M % BM or N % BN or K % (2 * BK):
        raise ValueError(f"({M},{N},{K}) not a multiple of ({BM},{BN},2 * BK)")
    n_tiles = K // BK
    ...
    class Shared:
        ...
        a1: fx.Array[fx.Float32, BK * BM, 16]   # the second stage
    ...
    def kernel(A: fx.Tensor, B: fx.Tensor, C: fx.Tensor):
        ...
        # LDS ping-pong: this tile's math reads buffer p while the next tile's
        # data is written into buffer 1-p, so the second barrier disappears --
        # one per K-tile instead of two.
        #
        # Unrolled by two so the buffer index stays a compile-time constant. The
        # trailing prefetch of tile n_tiles is clamped inside load_tile_to_regs
